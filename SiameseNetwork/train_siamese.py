...
class CosDist(Layer):
    ...
    def call(self, input_embedding, validation_embedding):
        ...
        denominator = tf.math.round(tf.math.multiply(
            tf.math.sqrt(sum_input), tf.math.sqrt(sum_validation)))
...
def make_siamese_model():
    ...
    distances = tf.keras.layers.Lambda(L2DistFunc)(
...
def train(data, EPOCHS):
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        print("Loss:", loss.numpy(), "Recall:",
              r.result().numpy(), "Precision:", p.result().numpy())
